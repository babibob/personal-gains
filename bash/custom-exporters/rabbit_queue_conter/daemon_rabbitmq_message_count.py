...
while(True):
    with open('/tmp/rabbit_queue_count-daemon.file.draft', 'w+') as draftfile:
        params = pika.ConnectionParameters(host='10.117.0.4',port=5672,credentials=pika.credentials.PlainCredentials('marketplace', 'Lesser6remedy9each'), )
        connection = pika.BlockingConnection(parameters=params)
        channel = connection.channel()

        def rest_queue_list(user='marketplace',password='Lesser6remedy9each',host='rabbitmq.marketplace.local.maf.io', port=443, virtual_host=None):
...
#            print(f'rabbitmq_queue_message_count{{host="rabbitmq.mobile.yc.maf.io", queue_name=\"{queue_name}\"}} {count_messages}')
            draftfile.write(f'rabbitmq_queue_message_count{{host="rabbitmq.marketplace.local.maf.io", queue_name=\"{queue_name}\"}} {count_messages}\n')
